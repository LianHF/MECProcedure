...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
    ...
    if(state & MF_SEPARATOR){
        ...
        if (Win32s!=g_Shell)
            ...
            size.cy=(LONG)(DrawText(pDC->m_hDC,(LPCTSTR)lpstrText,(int)wcslen(lpstrText),(LPRECT)(&rect),(UINT)(DT_SINGLELINE|DT_LEFT|DT_VCENTER|DT_CALCRECT)));
...
#ifdef _CPPRTTI 
        psubmenu=dynamic_cast<BCMenu *>(GetSubMenu((int)i));
#else
                                     ...
                                     CMenu *pMenu)
{
    ...
    if(pBCMenu && nFlags&MF_POPUP){
        ...
        key.SetAt(1,(TCHAR)nChar);//liujibbs
        key.MakeLower();
        CString menutext;
    ...
    if(MF_BYPOSITION&nFlags){
        ...
        if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
            ...
            if(NULL==pSubMenu){
                ...
                INT_PTR num = pSubMenu->GetMenuItemCount();
                INT_PTR i = 0;
                for(i=num-1;i>=0;--i)pSubMenu->DeleteMenu((UINT)i,MF_BYPOSITION);
                for(i=m_MenuList.GetUpperBound();i>=0;i--){
                    if(m_MenuList[i]->nID==(UINT)pSubMenu->m_hMenu){
                        delete m_MenuList.GetAt(i);
                        m_MenuList.RemoveAt(i);
