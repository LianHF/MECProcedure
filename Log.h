...
public:    
    ...
    {  
        ...
        sprintf(szPath, "%s", "C:");
		strcat(szPath, "\\MECProcedure_log\\");
		BOOL isExist = PathFileExists(szPath);
		if (!isExist) CreateDirectory(szPath, NULL);
		string lcFileName = GetLogName();
        strcat(szPath, lcFileName.c_str());  
    }  

	template <class T>  
	static void WriteLogA(T x)  
	{  
		CHAR szPath[MAX_PATH] = {0};  
		GetLogFilePathA(szPath);  
	...
	template <class T>  
	static void WriteLog4G(T x)  
	{  
		CHAR szPath[MAX_PATH] = {0};  
		GetLogFilePath4G(szPath);  
...
private:  
    ...
    static string GetLogName()
	{
		...
		tm* tLocalTime = localtime( &tNowTime);    
		char szTime[30] = {'\0'};    
		strftime(szTime, 30, "%Y-%m-%d.log", tLocalTime);    
		string strTime = szTime;    
		return strTime;
	}
};    
...
//用下面这些宏来使用本文件
#define LOGA(x)				CLog::WriteLogA(x);
